...
// Two ad placements per task archive — varied slot pairs so no two pages share
// the same combination. [top-of-results, mid/bottom-of-results].
const archiveAdSlots: Record<TaskKey, [string, string]> = {
  article: ['header', 'in-feed'],
  profile: ['header', 'footer'],
  listing: ['in-feed', 'footer'],
  image: ['header', 'sidebar'],
  classified: ['header', 'in-feed'],
  sbm: ['header', 'footer'],
  pdf: ['in-feed', 'footer'],
}

function AdBlock({ slot, eager = false, className = '' }: { slot: string; eager?: boolean; className?: string }) {
  return (
...
const cardBase = 'group block rounded-[var(--tk-radius)] border border-[var(--tk-line)] bg-[var(--tk-surface)] transition duration-500 hover:-translate-y-1.5 hover:border-[var(--tk-gold-soft)] hover:shadow-[0_30px_72px_rgba(0,0,0,0.6)]'
...
export function TaskArchiveView({ task, posts, pagination, category, basePath }: { task: TaskKey; posts: SitePost[]; pagination: SiteFeedPagination; category: string; basePath: string }) {
  ...
  const [adTop, adMid] = archiveAdSlots[task] || ['header', 'in-feed']
      ...
      <main style={taskThemeStyle(task)} className="min-h-screen bg-[var(--tk-bg)] text-[var(--tk-text)]">
        <header className="relative overflow-hidden border-b border-[var(--tk-line)]">
          ...
          <div className="relative mx-auto max-w-[var(--editable-container)] px-6 py-20 sm:py-28 lg:px-8">
            <div className="flex items-center gap-3 text-[11px] font-semibold uppercase tracking-[0.34em] text-[var(--tk-gold)]">
              <span>{theme.kicker}</span>
              <span className="h-1 w-1 rounded-full bg-[var(--tk-gold)] opacity-60" />
        ...
        <AdBlock slot={adTop} eager />

        <section className="mx-auto max-w-[var(--editable-container)] px-6 pb-16 pt-6 sm:pb-20 lg:px-8">
          {posts.length ? (
          ...
          <AdBlock slot={adMid} className="!max-w-5xl" />
  ...
  return (
    <div className={`mt-2.5 flex items-center gap-2 ${center ? 'justify-center' : ''}`}>
      <span className="inline-flex items-center gap-[3px]">
        {[0, 1, 2, 3, 4].map((i) => (
          <Star key={i} className={`h-4 w-4 ${i < filled ? 'fill-[var(--tk-gold)] text-[var(--tk-gold)]' : 'fill-transparent text-[var(--tk-muted)] opacity-40'}`} />
        ))}
      </span>
...
function ImageArchiveCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  const image = getImage(post)
  return (
    <Link href={href} className="group mb-5 block break-inside-avoid overflow-hidden rounded-[var(--tk-radius)] border border-[var(--tk-line)] bg-[var(--tk-surface)] transition duration-500 hover:-translate-y-1.5 hover:border-[var(--tk-gold-soft)] hover:shadow-[0_30px_72px_rgba(0,0,0,0.6)]">
      <div className={`relative overflow-hidden ${index % 3 === 0 ? 'aspect-[3/4]' : 'aspect-[4/3]'}`}>
        ...
        <div className="absolute inset-0 bg-[linear-gradient(180deg,transparent_40%,rgba(15,15,14,0.9))] opacity-85 transition group-hover:opacity-100" />
        <div className="absolute inset-x-0 bottom-0 p-5">
          ...
          <span className="mt-2 inline-flex items-center gap-1.5 text-xs font-semibold text-[var(--tk-gold)]">View image <ArrowUpRight className="h-3.5 w-3.5" /></span>
        </div>
      </div>
    </Link>
  )
...
function ProfileArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className={`${cardBase} flex flex-col items-center p-7 text-center`}>
      <div className="flex h-24 w-24 items-center justify-center overflow-hidden rounded-full border-2 border-[var(--tk-gold-soft)] bg-[var(--tk-raised)] ring-2 ring-[var(--tk-gold-soft)] ring-offset-2 ring-offset-[var(--tk-surface)]">
